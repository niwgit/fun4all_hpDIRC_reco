...
void geo_reco_epi_f4a(TString infile, TString lutfile, TString filedir, int verbose)
{
  fVerbose = verbose;
  ...
  TH1F* Hist_track_theta_piplus = new TH1F("track_theta_piplus",";Polar angle of track in the bar (deg)", 100, 20, 40);
  TH1F* Hist_track_phi_piplus = new TH1F("track_phi_piplus",";Azimuthal angle of track in the bar (deg)", 720, -180, 180);
  TH1F* Hist_track_theta_eplus = new TH1F("track_theta_eplus",";Polar angle of track in the bar (deg)", 100, 20, 40);
  TH1F* Hist_track_phi_eplus = new TH1F("track_phi_eplus",";Azimuthal angle of track in the bar (deg)", 720, -180, 180);
  ...
  if(!gSystem->AccessPathName(fCorrFile)){
    ...
    for(int i=0; i<ch.GetEntries(); i++){
      ...
      std::cout << "pmt " << pmt << "  " << fCorr[pmt] << " spr = (0) " << fSigma[0] << "  (2) " << fSigma[2] << std::endl;
    }
  }else{
  ...
  int track_id[arr_size], track_pid[arr_size];
  ...
  for (int ievent=0; ievent < nEvents; ievent++){
	...
	for(auto iter2 = iter->second.begin(); iter2 != iter->second.end(); ++iter2)
	  {
	    avg_track_momentum += *iter2;
	  }
	avg_track_momentum *= 1 / (Double_t) iter->second.size();
	//avg_track_momentum.Print();

	vec_track_id.push_back(iter->first);
	vec_avg_track_mom.push_back(avg_track_momentum);
      }	
    ...
    if(ievent==0)
      {
        for(int h=0; h<5; h++)
	  {
	    fLnDiff[h] = new TH1F(Form("LnDiff_%d",h),  ";ln L(#pi) - ln L(e);entries [#]",300,-300,300);
	...
	double phi_track = mom_vec.Phi();

	if(pid == 0)
	  {
	    Hist_track_theta_eplus->Fill(mom_vec.Theta()*TMath::RadToDeg());
	    Hist_track_phi_eplus->Fill(phi_track*TMath::RadToDeg());
	  }
	if(pid == 2)
	  {
	    Hist_track_theta_piplus->Fill(mom_vec.Theta()*TMath::RadToDeg());
            Hist_track_phi_piplus->Fill(phi_track*TMath::RadToDeg());
	  }
	
	if(phi_track > 0) barbox_number = (int) (phi_track*TMath::RadToDeg()+15.0)/30.0; 
	if(phi_track < 0) barbox_number = (int) (phi_track*TMath::RadToDeg()-15.0)/30.0;

	//std::cout << "track in bar box = " << barbox_number << std::endl;
    ...
    std::cout << "fLnDiff[fp2] entries = " << fLnDiff[fp2]->GetEntries() << "\t" << "fLnDiff[fp1] entries = " << fLnDiff[fp1]->GetEntries() << std::endl;
	...
	fLnDiff[fp2]->Fit("gaus","S","",m1-1.8*s1,300);
	ff = fLnDiff[fp2]->GetFunction("gaus");
	m1=ff->GetParameter(1);
	s1=ff->GetParameter(2);
	dm1=ff->GetParError(1);
    ...
    if(fLnDiff[fp1]->GetEntries()>10){
      ...
      if(fp1==0 && mom <1.5){ ///handle tails
	fLnDiff[fp1]->Fit("gaus","S","",-300,m2+1.8*s2);
	ff = fLnDiff[fp1]->GetFunction("gaus");      
	m2=ff->GetParameter(1);
	s2=ff->GetParameter(2);
	dm2=ff->GetParError(1);
  ...
  Hist_lut_time->Write();
  Hist_track_theta_piplus->Write();
  Hist_track_phi_piplus->Write();
  Hist_track_theta_eplus->Write();
  Hist_track_phi_eplus->Write();

  prt_theta = Hist_track_theta_piplus->GetMean();
